...
namespace WiimoteLib
{
	...
	public class Wiimote : IDisposable
	{
		...
		internal delegate bool WiimoteFoundDelegate(string devicePath, string serial);
		...
		internal Wiimote(string devicePath, string serial)
		{
		...
		internal static void FindWiimote(WiimoteFoundDelegate wiimoteFound)
		{
			...
			while(HIDImports.SetupDiEnumDeviceInterfaces(hDevInfo, IntPtr.Zero, ref guid, index, ref diData))
			{
				...
				{
					Debug.WriteLine(string.Format("{0}: {1} - {2}", index, diDetail.DevicePath, new Win32Exception(Marshal.GetLastWin32Error()).Message));
					...
					if(HIDImports.HidD_GetAttributes(mHandle.DangerousGetHandle(), ref attrib))
					{
						...
						if(attrib.VendorID == VID && attrib.ProductID == PID)
						{
                            ...
                            Byte[] serialBuffer = new Byte[126];
                            var serial = string.Empty;

                            if (HIDImports.HidD_GetSerialNumberString(mHandle.DangerousGetHandle(), serialBuffer, 126))
                            {                                
                                serial = System.Text.Encoding.Unicode.GetString(serialBuffer).TrimEnd(new char[]{(char)0});
                                Debug.WriteLine(string.Format("{0}: {1} - {2}", index, diDetail.DevicePath, serial)); 
                            }

                            Debug.WriteLine(string.Format("{0}: {1} - {2}", index, diDetail.DevicePath, new Win32Exception(Marshal.GetLastWin32Error()).Message)); 

							// fire the callback function...if the callee doesn't care about more Wiimotes, break out
							if(!wiimoteFound(diDetail.DevicePath, serial))
								break;
						}
					}
					mHandle.Close();
		...
		private bool WiimoteFound(string devicePath, string serial)
		{
			mDevicePath = devicePath;
...
//					mWiimoteState.ClassicControllerState.AccelCalibrationInfo.MaxTriggerR = (byte)(buff[15] >> 3);
        ...
        /// <summary>
        /// The manufacturer serial number for this Wiimote
        /// </summary>
